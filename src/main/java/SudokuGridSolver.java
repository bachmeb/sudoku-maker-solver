...
public class SudokuGridSolver {
    ...
    public SudokuGrid solve(SudokuGrid grid) {
        ...
        while (true) {
            ...
            gotOne = solveByEliminationInAdjacentBoxesHorizontal();
            if (gotOne) {
                logger.info("got one");
                continue;
            }

            gotOne = solveByEliminationInAdjacentBoxesVertical();
            if (gotOne) {
                logger.info("got one");
                continue;
            }
    ...
    private boolean solveByEliminationInAdjacentBoxesHorizontal() {
    ...
    private boolean solveByEliminationInAdjacentBoxesVertical() {
            ...
            for (int boxIndex = 0; boxIndex < grid.getBoxes().length; boxIndex++) {
                ...
                if (!numberInMindIsInBox) {
                    ...
                    for (int vBoxIndex : adjacentBoxesVertical) {
                        boolean itsThere = checkSetForNumber(numberInMind, grid.getBoxes()[vBoxIndex]);
                        if (itsThere) {
                            count++;
                        }
                    }
